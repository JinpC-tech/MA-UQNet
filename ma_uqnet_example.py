...
def generate_synthetic_data():
    ...
    stages = np.random.randint(0, N_STAGES, N_SAMPLES)
    ...
    stage_spectral_means = [
        np.array([450, 480, 520, 500, 380, 520, 580, 850, 720, 1800, 420, 800]),
        np.array([480, 510, 560, 540, 410, 560, 620, 920, 780, 1900, 450, 860]),
        np.array([520, 540, 600, 580, 450, 600, 670, 1000, 850, 2050, 490, 930]),
        np.array([500, 530, 580, 560, 430, 580, 650, 970, 810, 1980, 470, 900])
    ]
    
    stage_env_means = [
        np.array([45, 35, 250, 320, 25]),
        np.array([55, 48, 350, 450, 35]),
        np.array([65, 62, 480, 620, 48]),
        np.array([75, 78, 620, 800, 62])
    ]
    
    stage_biomass_base = [800, 1500, 2500, 3200]
...
def preprocess_data():
    ...
    for stage_id in range(N_STAGES):
        ...
        print(f"  Stage {stage_id}: {count} samples ({count/N_SAMPLES*100:.1f}%) - "
...
class GrowthStageAwareModule(nn.Module):
    def __init__(self, num_stages, embedding_dim=32):
        super().__init__()
    ...
    def forward(self, stages):
        ...
        stages_flat = stages.view(-1)
        stage_embedding = self.stage_embedding(stages_flat)
        
        one_hot_stages = torch.zeros(batch_size, self.num_stages, device=stages.device)
        one_hot_stages.scatter_(1, stages, 1)
...
class MAUQNet(nn.Module):
    def __init__(self, spectral_dim, env_dim, hidden_dim, fusion_dim, 
                 num_stages, dropout_rate):
        super().__init__()
        ...
        self.growth_stage_module = GrowthStageAwareModule(num_stages, embedding_dim=hidden_dim)
...
def main():
    ...
    print("Example Implementation - Using Synthetic Data")
    print("="*80)
