...
# Zadoks Z-Scale Primary Growth Stages (0-9)
GROWTH_STAGES = {
    0: "Germination",
    1: "Leaf Development",
    2: "Tillering",
...
SELECTED_STAGES = [3, 4, 5, 6]  # Jointing, Booting, Flowering, Milk Development
...
def generate_synthetic_data():
    ...
    stage_spectral_means = {
        3: np.array([520, 540, 600, 580, 450, 600, 670, 1000, 850, 2050, 490, 930]),      # Jointing
        4: np.array([540, 560, 620, 600, 470, 620, 690, 1050, 880, 2100, 510, 960]),      # Booting
        5: np.array([560, 580, 640, 620, 490, 640, 710, 1100, 910, 2150, 530, 990]),      # Flowering
        6: np.array([580, 600, 660, 640, 510, 660, 730, 1150, 940, 2200, 550, 1020])      # Milk Development
    }
    
    # Environmental conditions for each stage
    stage_env_means = {
        3: np.array([65, 62, 480, 620, 48]),      # Jointing
        4: np.array([70, 70, 550, 720, 55]),      # Booting
        5: np.array([75, 78, 620, 800, 62]),      # Flowering
        6: np.array([80, 85, 680, 880, 70])       # Milk Development
    }
    
    # Biomass baseline for each stage (kg/ha)
    stage_biomass_base = {
        3: 2500,    # Jointing
        4: 3200,    # Booting
        5: 3800,    # Flowering
        6: 4200     # Milk Development
    }
...
class GrowthStageSpecificModule(nn.Module):
    def __init__(self, embedding_dim=32):
        super().__init__()
        # Embedding for all 10 Zadoks stages (0-9)
        # This allows direct input of Zadoks stage indices without mapping
    ...
    def forward(self, stages):
        ...
        # Directly use Zadoks stage indices (no mapping needed)
        stage_embedding = self.stage_embedding(stages_flat)
        
        # Create one-hot encoding for the 10 stages
    ...
    def __init__(self, spectral_dim, env_dim, hidden_dim, fusion_dim, dropout_rate):
        ...
        self.stage_specific_layers = nn.ModuleList([
            nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(dropout_rate)
            ) for _ in range(10)  # 10 Zadoks stages (0-9)
        ])
